...
class ClientRequestsService {
  ...
  // ESTA ES LA QUE CORREGIMOS PARA QUE NO FALLE AL ASIGNAR
  Future<Resource<bool>> updateDriverAssigned(int idClientRequest, int idDriver, double fareAssigned) async {
    try {
      ...
      print('📡 UPDATE DRIVER STATUS: ${response.statusCode}');
      ...
      print('❌ Error crítico en updateDriverAssigned: $e');
      return ErrorData(e.toString());
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (data is List) {
           ...
           return ErrorData("Formato incorrecto");
        }
      } else {
        String message = 'Error desconocido';
        if (data is Map && data.containsKey('message')) {
           message = listToString(data['message']);
        }
  ...
  Future<Resource<ClientRequestResponse>> getByClientRequest(int idClientRequest) async {
    try {
      ...
      print('🔍 GET CLIENT REQUEST (${idClientRequest}) STATUS: ${response.statusCode}');
      print('🔍 GET CLIENT REQUEST BODY: ${response.body}');
      // -----------------------------------------

      final data = json.decode(response.body);

      if (response.statusCode == 200 || response.statusCode == 201) {
        // Verificación de seguridad: ¿Es realmente un Mapa?
        if (data is Map<String, dynamic>) {
           ClientRequestResponse clientRequest = ClientRequestResponse.fromJson(data);
           return Success(clientRequest);
        } else {
           print('❌ Error: Se esperaba un Mapa JSON, se recibió: ${data.runtimeType}');
           return ErrorData('Formato de respuesta incorrecto');
        }
      }
      else {
        return ErrorData(listToString(data['message']));
      ...
      print('❌ Error CRÍTICO en getByClientRequest: $e');
      return ErrorData(e.toString());
    }
  }

  Future<Resource<bool>> cancelRequest(int idClientRequest) async {
    try {
      Uri url = Uri.http(ApiConfig.API_PROJECT, '/client-requests/$idClientRequest');
      Map<String, String> headers = { 'Content-Type': 'application/json' };
