...
export class ClientRequestsService extends Client {
    ...
    async create(clientRequest: CreateClientRequestDto) {
        try {
            ...
            this.socketGateway.server.emit('created_request_client', { data: { ...clientRequest, id: newRequestId } });

            // Notificar vía Firebase
    ...
    // --- ESTE ES EL MÉTODO CORREGIDO CON EL FILTRO ---
    async getNearbyTripRequest(driver_lat: number, driver_lng: number) {
        ...
        WHERE
            ...
            AND CR.status = '${Status.CREATED}' -- Filtro de estado
        ...
        if(data.length > 0) {
            const pickup_positions = data.map((d: any) => ({ 
    ...
    async delete(id: number) {
        try {
            ...
            // 2. EMITIMOS EL EVENTO PARA QUE DESAPAREZCA DE LOS CONDUCTORES
            // Usaremos el mismo canal que ya configuramos en el Driver: 'new_driver_assigned'
            // O podemos crear uno nuevo 'deleted_client_request'.
            // Para no complicarte editando el driver de nuevo, vamos a emitir 'created_request_client'
            // con un flag de borrado o mejor, emitimos un evento de borrado específico.
            
            // RECOMENDACIÓN: Emitir un evento de borrado
